...
| `-sv solver`, `--solver solver`      | Specifies the solver to use. Available only for `CP`, `SMT` and `MIP`.                                                                                            | CP: `{gecode, chuffed}` <br> SMT: `{z3, msat, yices, cvc5}` <br> MIP: `{CBC, SCIP, HIGHS, GUROBI}` <br> Default: `all` (solves using all solvers)         |
